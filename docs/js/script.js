...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    let statusBar = document.getElementById('real-time-extra');
    let valuebar = document.getElementById('real-time-value');

    if (typeof window.ethereum === 'undefined') {
      statusBar.innerText = 'Metamask not installed';
      return
    }
    ...
    .then(accounts => {
      if (accounts == 0) {
        statusBar.innerHTML = 'Metamask not connected<br><button id="real-time-connect" type="button">connect</button>';
        document.getElementById('real-time-connect').onclick = function(){
          ethereum.request({ method: 'eth_requestAccounts' }).then(accounts => {if (accounts > 0) handleAccounts(accounts);});
        };
      } else handleAccounts(accounts);
    });
  })();
